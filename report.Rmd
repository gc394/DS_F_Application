...
This approach will allow users to do is to flagged submissions quickly, if we take Firm 139's *scr_m* submissions we can see that they have a range of flags throughout the series.

```{r Outlier Firm 12 table example, echo = FALSE}

df %>%
  dplyr::filter(firm == 'Firm 139', value_type == 'time_series_value') %>%
  dplyr::select(firm, year, scr_m) %>%
  knitr::kable() %>%
  kable_styling("striped", full_width = F)
...
firm12_versus_average_df = rbind(
df %>%
  dplyr::filter(firm == 'Firm 139', value_type == 'time_series_value') %>%
  dplyr::select(firm, year, scr_m) %>%
  dplyr::mutate(text = outliers_df[outliers_df$firm == 'Firm 139',]$scr_m),
df %>%
    dplyr::filter(firm != 'Firm 139', value_type == 'time_series_value') %>%
    dplyr::group_by(year) %>%
    dplyr::summarise(scr_m = median(scr_m, na.rm = T),
                     text = '',
                     firm = 'Agg')
)
...
ggplot2::ggplot(
  ...
  # sets colors on for topn firms
  scale_color_manual(
    values = c('Firm 139' = ggthemes::wsj_pal()(1),
               'Agg' = ggthemes::wsj_pal()(2)[2]), 
    name = 'Submission'
  ) +
  scale_y_continuous(#scales::label_number_auto()
    labels = scales::label_number_auto()) + 
  labs(
    title = glue::glue('Firm 139 submission scr_m time series'),
