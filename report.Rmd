...
```{r Missing Firms, echo=FALSE, include = F}
...
# pretty table output
knitr::kable(modified_z_df)  %>%
  kable_styling("striped", full_width = F) %>% 
  scroll_box(width = "100%")

```

I have created a table here that provides a potential list of the top 10 firms. I have calculated this by taking the *mean modified z score* for *Net Weighted Premium*, *Net BEL (inc. TPs as a whole, pre-TMTP)* and *Pure Gross Claims Ratio*. Firstly, I chose to use the modified z-score (you can find more [here](https://www.ibm.com/docs/en/cognos-analytics/11.1.0?topic=terms-modified-z-score)) as it is more robust to extreme outliers than the standard z score and it gives us a good idea on the position of the metrics for each firm when compared to the populatin. I chose those three variables because collectively these are the three variables that are highly positively correlated with every variable in the dataset and so roughly they will contain the information of the dataset. As yiou can see this needs to be read with some sceptiscm still as there are major outliers that disrupt the results, for example Firm 188's *Pure Gross Claims Ratio* value.
...
outliers_df %>%
  dplyr::filter(firm %in% c('Firm 139', 'Firm 222')) %>%
...
```{r Outlier Firm 139 chart example, echo = FALSE, warning=FALSE}

plot_outlier_firm_vs_average = function(df, example_firm, metric){
  
  firm_versus_average_df = plyr::rbind.fill(
  df %>%
  ...
  df %>%
      dplyr::filter(firm != example_firm, value_type == 'time_series_value') %>%
      dplyr::group_by(year) %>%
      dplyr::summarise(median = median(.data[[metric]], na.rm = T),
                       upper_bound = quantile(.data[[metric]], 0.75, na.rm = T) + (1.5 * IQR(.data[[metric]], na.rm = T)),
                       lower_bound = quantile(.data[[metric]], 0.25, na.rm = T) - (1.5 * IQR(.data[[metric]], na.rm = T)),
                       firm = 'Agg')
  )
...
# provide the cluster positions of each group by creating a tidy tibble
clusters <- 
  kclusts %>%
...
# farm out the clustering summary measures
clusterings <- 
  kclusts %>%
...
ggplot(assignments, 
  ...
  facet_wrap(~ k) +
  labs(
    title = 'K-means Clusters from the Principal Components', 
...
This chart above shows us how the groups form with different values of K, due to the shape of the population (part due to cleaning outliers and using median values instead of investigating temporally) the groups begin difficult to disentagle when K > 5 when only looking at PCs 1 and 2. 

```{r Total Withinss, echo = FALSE}

ggplot(clusterings, aes(k, tot.withinss)) +
  geom_line() +
  geom_text(mapping = aes(label = round(tot.withinss, 1)), nudge_y = 50) +
